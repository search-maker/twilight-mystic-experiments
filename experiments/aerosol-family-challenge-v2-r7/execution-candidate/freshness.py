...
def authorization_branch(ordinal: int) -> str:
    return f'authorization/aerosol-family-challenge-v2-r7-ordinal-{ordinal}'


def dispatch_branch(ordinal: int) -> str:
    return f'dispatch/aerosol-family-challenge-v2-r7-ordinal-{ordinal}'


def execution_key(ordinal: int) -> str:
    return f'aerosol-family-challenge-v2-r7:numerical:{ordinal}'


def authorization_marker(ordinal: int, head: str, parent: str, pr_number: int) -> str:
    return (
        f'ORDINAL{ordinal}_AEROSOL_FAMILY_V2_R7_AUTHORIZATION_ALLOCATED_REVIEWED_NOT_DISPATCHED '
        f'commit={head} parent={parent} pr={pr_number}'
    )


def consumed_marker(ordinal: int) -> str:
    return f'ORDINAL{ordinal}_AEROSOL_FAMILY_V2_R7_DISPATCH_CONSUMED'


def marker_regex(ordinal: int) -> re.Pattern[str]:
    return re.compile(
        rf'^ORDINAL{ordinal}_AEROSOL_FAMILY_V2_R7_AUTHORIZATION_ALLOCATED_REVIEWED_NOT_DISPATCHED '
