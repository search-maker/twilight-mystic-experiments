...
def evaluate(
    ...
    _guard_require(context.get('githubActions') is True and context.get('eventName') == 'workflow_dispatch' and context.get('runAttempt') == 1, 'not exact attempt-1 GitHub workflow_dispatch context')
    ...
    _guard_require(context.get('authorizationCommitChangedPaths') == ['experiments/aerosol-family-challenge-v2-r8/authorization.json'], 'authorization commit changed unexpected paths')
    ...
    _guard_require(isinstance(prior_artifacts, list) and not any(str(x).startswith('aerosol-family-v2-r8-case-') for x in prior_artifacts), 'prior aerosol-family case artifact exists')
    publisher = context.get('dispatchPublisher') or {}
    _guard_require(publisher.get('status') == 'DISPATCH_PUBLISHER_PASS_ACTUAL_GIT_PUSH', 'successful dispatch publisher evidence missing')
    _guard_require(publisher.get('authorizationHead') == head and publisher.get('authorizationParent') == parent and publisher.get('authorizationPr') == pr.get('number'), 'dispatch publisher authorization identity drift')
    _guard_require(publisher.get('dispatchBranch') == authorization.get('dispatchBranch') and publisher.get('dispatchBranchHead') == head, 'dispatch publisher branch/head drift')
    _guard_require(publisher.get('actualGitPush') is True and publisher.get('currentConsumedMarkerPosted') is True, 'actual git push / consumed marker proof missing')
    _guard_require(publisher.get('githubTokenPushReliedUponToTriggerScience') is False and publisher.get('scienceTriggerMode') == 'EXPLICIT_WORKFLOW_DISPATCH_AFTER_ACTUAL_GIT_PUSH', 'publisher trigger-mode drift')
    _guard_require(publisher.get('runAttempt') == 1 and publisher.get('conclusion') == 'success', 'publisher must be one successful attempt-1 run')
    _guard_require(publisher.get('scientificRuntimeSetupPerformed') is False and publisher.get('solverExecutionPerformed') is False, 'publisher performed scientific runtime')
    comments = context.get('issue60Comments')
    marker = consumed_marker(ordinal)
    consumed=[str(x).strip() for x in comments] if isinstance(comments,list) else []
    _guard_require(sum(1 for x in consumed if x.lower() == marker.lower()) == 1, 'exactly one dispatch consumed marker required before science')
    ...
    return {
        'schemaVersion': 2,
        'stageId': 'aerosol-family-challenge-v2-r8-execution-guard',
        'status': 'EXACT_ONE_USE_AEROSOL_FAMILY_V2_R8_DISPATCH_AUTHORIZED',
