...
STAGE_ID='aerosol-family-challenge-v2-r8-freeze'
EXACT_BLOBS={
...
def _verify_science_bytes(package: Path, analysis_contract: Path) -> dict:
    if analysis_contract.resolve() != (package/'analysis-contract.v3.json').resolve():
        raise Refusal('R8 analysis contract must be the bound local copy')
    for name,expected in EXACT_BLOBS.items():
        path=package/name
        if not path.is_file() or git_blob_sha1(path)!=expected:
            raise Refusal(f'R8 bound scientific byte drift: {name}')
    ...
    for key,value in exact.items():
        if contract.get(key)!=value:
            raise Refusal(f'R8 analysis contract boundary drift: {key}')
    return contract
