...
The fenced two-pass fingerprint is a deterministic canonical representation of the complete collision-relevant first-pass universe: stable row identities plus all non-operational fields/content, with rows and nested arrays sorted deterministically. Only inherently mutable lifecycle metadata (timestamps, status/state, conclusion and retry/expiry fields) is omitted. GitHub's asynchronously populated open-PR `merge_commit_sha` is also omitted because it is a synthetic mergeability/test-merge value rather than the PR head identity; the actual PR head ref/SHA, base identity, title/body and all candidate-seed-bearing content remain fingerprinted. Thus harmless pagination order, lifecycle drift, synthetic test-merge calculation, and non-seed post-fence arrivals are tolerated, while any fenced branch/head, artifact-name, PR/issue body, comment, or candidate-seed change remains a stability failure and stays fail-closed.
