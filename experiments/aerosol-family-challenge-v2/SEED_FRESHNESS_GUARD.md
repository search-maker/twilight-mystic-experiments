...
The repository-global collector performs two complete paginated enumerations. After removing only the current audit run row and artifact metadata created by that same run, the canonical JSON bytes of the two external snapshots must be identical. This prevents a page-shift race from silently omitting a new branch/run/artifact/PR/issue/comment while the audit is in progress.
...
The two-pass fingerprint is a deterministic canonical representation of the complete collision-relevant surfaces: stable row identities plus all non-operational fields/content, with rows and nested arrays sorted deterministically. Only inherently mutable lifecycle metadata (timestamps, status/state, conclusion and retry/expiry fields) is omitted. Thus harmless pagination order or lifecycle drift is tolerated, while any branch/head, artifact-name, PR/issue body, comment, or candidate-seed change remains a stability failure and stays fail-closed.
