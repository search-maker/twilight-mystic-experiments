# Execution candidate — R6 review-only, not authorized
...
The case executor requires both explicit execution opt-in and a guard report with status `EXACT_ONE_USE_AEROSOL_FAMILY_V2_DISPATCH_AUTHORIZED`. The guard requires post-dispatch freshness, zero prior dispatch use, zero prior aerosol-family case artifacts, no consumed marker, and all closed protected/fitting/production boundaries.
