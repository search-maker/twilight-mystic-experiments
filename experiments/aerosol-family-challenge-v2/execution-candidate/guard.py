...
def evaluate(
    ...
    _guard_require(context.get('authorizationCommitChangedPaths') == ['experiments/aerosol-family-challenge-v2/authorization.json'], 'authorization commit changed unexpected paths')
    ...
    _guard_require(isinstance(prior_artifacts, list) and not any(str(x).startswith('aerosol-family-v2-case-') for x in prior_artifacts), 'prior aerosol-family case artifact exists')
    ...
    return {
        'schemaVersion': 2,
        'stageId': 'aerosol-family-challenge-v2-execution-guard',
