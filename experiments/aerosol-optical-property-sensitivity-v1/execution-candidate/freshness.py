...
def validate_preauthorization(ctx: dict[str, Any], ordinal: int) -> None:
    ...
    require((not auth_exists) or reusable, "authorization branch already exists and is not proven reusable after failed review")
    require(ctx.get("activeAuthorizationPathOnMainExists") is False, "active AOPS authorization path already exists on main")
