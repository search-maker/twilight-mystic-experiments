...
LOCAL_FRESHNESS_BLOB = "d8767f731fef58161034b92356906447d725ce62"
...
def _failed_history_must_be_absent(payload: dict[str, Any], ordinal: int) -> dict[str, Any]:
    pattern = re.compile(rf"^history/{re.escape(STAGE)}-ordinal-{ordinal}-auth-review-failed-([1-9][0-9]*)$", re.I)
    found = [str(row.get("name") or "") for row in payload.get("branches", []) if pattern.fullmatch(str(row.get("name") or ""))]
    if found:
        raise SurfaceRefusal("ASIV failed-authorization history exists; separately reviewed recovery is required")
    return {"heads": [], "prNumbers": [], "reviewRunIds": []}


def _modules():
    freshness = load_bound("asiv_freshness_for_control", HERE / "freshness.py", LOCAL_FRESHNESS_BLOB)
    control = load_bound("asiv_bound_aops_control_surface", AOPS_DIR / "control_surface.py", AOPS_CONTROL_BLOB)
    ordinal = load_bound("asiv_bound_aops_global_ordinal", AOPS_DIR / "global_ordinal.py", AOPS_GLOBAL_ORDINAL_BLOB)
...
def _geometry(surface: dict[str, Any], passed: bool) -> dict[str, Any]:
    surface["candidateGeometryAuthorizationRecheckPassed"] = bool(passed)
    return surface
...
def build_authorization_review_surface(
    ...
    candidate_geometry_authorization_recheck_passed: bool,
) -> dict[str, Any]:
    ...
    _geometry(surface, candidate_geometry_authorization_recheck_passed)
