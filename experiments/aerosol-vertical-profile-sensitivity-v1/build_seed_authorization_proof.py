...
def build(
    ...
    seed_mod = load_module("avps_seed_ledger_for_authorization_proof", stage_dir / "seed_ledger.py")
    ...
    if ledger.get("trackedCandidateSeedLedger") is not False:
        raise Refusal("candidate seed literals unexpectedly tracked by ledger contract")
    ...
    if tracked_tree_report.get("requiredSelfLedgerPathsPresent") is not True:
        raise Refusal("tracked-tree self-ledger policy did not pass")
    ...
    if global_report.get("auditedBranchHeadMatchesRepositoryHead") is not True:
        raise Refusal("repository-global audited main head mismatch")
    ...
    if int(global_report.get("priorReviewProofArtifactCount") or 0) < 1:
        raise Refusal("no prior vertical-profile review-freeze proof artifact visible at authorization time")
    ...
    return {
        ...
        "priorReviewProofArtifactCount": global_report.get("priorReviewProofArtifactCount"),
        "reviewProofArtifactName": global_report.get("reviewProofArtifactName"),
