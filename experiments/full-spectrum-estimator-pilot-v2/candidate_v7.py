...
CONTRACT=HERE/'transport-contract.v7.json'
BINDING=HERE/'review-binding.v6.json'
TEMPLATE=HERE/'authorization-template.ordinal15.json'
AUTH_BRANCH='authorization/full-spectrum-estimator-pilot-v2-ordinal15'
DISPATCH_BRANCH='dispatch/full-spectrum-estimator-pilot-v2-ordinal15'
EXECUTION_KEY='full-spectrum-estimator-pilot-v2:numerical:15'
TITLE='Full-spectrum estimator pilot v2 ordinal 15'
CANDIDATE_ORDINAL=15
PRIOR_ORDINAL=14
SCIENTIFIC_WORKFLOW='.github/workflows/full-spectrum-estimator-pilot-v2-ordinal15-execution-v7.yml'
AUTHORIZATION_REVIEW_WORKFLOW='.github/workflows/full-spectrum-estimator-pilot-v2-authorization-review-v7.yml'
MARKER_RE=re.compile(r'^ORDINAL15_AUTHORIZATION_ALLOCATED_REVIEWED_NOT_DISPATCHED commit=([0-9a-f]{40}) parent=([0-9a-f]{40}) pr=([1-9][0-9]*)$',re.I)
ORDINAL=r'ordinal\s*[-:#]?\s*15'
...
RESET=re.compile(r'(?:[.;]\s*|,\s*(?:and|but|then)\b|,\s*(?=ordinal\s*[-:#]?\s*15\b)|\b(?:but|however|yet|nevertheless|nonetheless|whereas|then|while)\b|\band\s+(?=(?:we\b|i\b|they\b|the\b|this\b|is\b|was\b|were\b|has\b|have\b|will\b|now\b|ordinal\s*[-:#]?\s*15\b|authorization\b|allocation\b|reservation\b|dispatch\b)))',re.I)
INTERROGATIVE=re.compile(rf'^\s*(?:(?:did|do|does)\s+{SUBJECT}\b|(?:is|are|was|were|has|have|had|can|could|would|should|will|may|might|must)\s+(?:{ORDINAL}|{SUBJECT}|{NOUN})\b|(?:who|what|when|where|why|how)\b)',re.I)
...
NONFACTUAL_PREFIX=re.compile(r'\b(?:if|whether|when|before|hope(?:d|s)?|wish(?:ed|es)?|want(?:ed|s)?|expect(?:ed|s)?|plan(?:ned|s)?|intend(?:ed|s)?|propose(?:d|s)?|false\s+that|not\s+established\s+that|not\s+proven\s+that|not\s+confirmed\s+that)\b',re.I)
FACT_PATTERNS=[
...
NOUN_MATCH_NEGATIVE=re.compile(r'\b(?:no|not|never|without|pending|requested|request|proposed|proposal|planned|plan|considered|discussion|review|denied|rejected|refused|revoked|rescinded|cancelled|canceled|unauthorized|unallocated|unreserved)\b',re.I)
...
def _reset_bounds(line:str,pos:int)->tuple[int,int]:
    resets=list(RESET.finditer(line)); before=[m.end() for m in resets if m.end()<=pos]; after=[m.start() for m in resets if m.start()>=pos]
    return max(before,default=0),min(after,default=len(line))
def _match_is_nonfactual(line:str,match:re.Match[str])->bool:
    a,b=_reset_bounds(line,match.start()); clause=line[a:b]; prefix=line[a:match.start()]
    return clause.rstrip().endswith('?') or bool(INTERROGATIVE.match(clause)) or bool(DIRECT_NEGATIVE.search(prefix)) or bool(NONFACTUAL_PREFIX.search(prefix))
def _assertive_markdown_prose(text:str)->str:
    out=[]; in_fence=False; fence=None
    for raw in (text or '').splitlines():
        s=raw.lstrip()
        if s.startswith('```') or s.startswith('~~~'):
            token=s[:3]
            if not in_fence: in_fence=True; fence=token
            elif token==fence: in_fence=False; fence=None
            continue
        if in_fence or s.startswith('>'): continue
        out.append(re.sub(r'`[^`\n]*`',' ',raw))
    return '\n'.join(out)
def positive_candidate_claims(text:str)->list[str]:
    claims=[]
    for raw in re.split(r'[\n]+|(?<=[.;])\s+',_assertive_markdown_prose(text)):
        line=raw.strip()
        if not line: continue
        if MARKER_RE.fullmatch(line): claims.append(line); continue
        if not ORDINAL_TOKEN.search(line): continue
        found=False
        for pat in FACT_PATTERNS:
            for match in pat.finditer(line):
                if not _match_is_nonfactual(line,match): claims.append(line); found=True; break
            if found: break
        if found: continue
        for pat in NOUN_FACT_PATTERNS:
            for match in pat.finditer(line):
                if not _match_is_nonfactual(line,match) and not NOUN_MATCH_NEGATIVE.search(match.group(0)):
                    claims.append(line); found=True; break
            if found: break
    return claims
...
def _is_candidate_scientific_run(run:dict[str,Any])->bool:
    ...
    return path==SCIENTIFIC_WORKFLOW or EXECUTION_KEY in title or title.lower()==TITLE.lower() or name.lower()==(TITLE+' execution v7').lower()
...
def collect(repository:str,token:str)->dict[str,Any]:
    ...
    runs=[]; page=1
    while True:
        v=transport_api._api(base+f'/actions/runs?per_page=100&page={page}',token); rows=v.get('workflow_runs',[]); runs.extend(rows)
        if len(rows)<100: break
        page+=1
    path='experiments/full-spectrum-estimator-pilot-v2/authorization.ordinal15.json'; exists,_=transport_api._exists(base+'/contents/'+urllib.parse.quote(path,safe='/')+'?ref=main',token)
...
def validate_common(ctx:dict[str,Any],dispatch_must_be_absent:bool=True)->None:
    require(ctx.get('latestPriorConsumedScientificOrdinal')==PRIOR_ORDINAL,'latest prior consumed scientific ordinal is not 14')
...
def validate_preauthorization(ctx:dict[str,Any])->None:
    ...
    require(ctx.get('nextAvailableScientificOrdinal')==CANDIDATE_ORDINAL,'ordinal 15 is not next available')
...
def main()->int:
    ...
    try:
        if a.cmd=='surface':
            ...
            c,b=validate_static(a.repository_root.resolve()); write_output({'status':'STATIC_BINDING_V7_PASS','reviewPathsVerified':len(b['reviewPaths'])+1,'caseCount':c['scientificPayload']['caseCount'],'scientificExecutionPerformed':False},a.output)
