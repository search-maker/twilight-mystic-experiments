...
PURPOSE = "jerusalem-tammuz-direct-mystic-v1"
EXPECTED_ANALYSIS_ID = "jerusalem-tammuz-direct-mystic-level-b-comparison-v1"
...
def analyze(manifest_path: Path, evidence_path: Path, contract_path: Path, cases_root: Path, derived_path: Path) -> dict[str, Any]:
    ...
    if contract.get("analysisId") != EXPECTED_ANALYSIS_ID:
    ...
    for group_id, geometry in geometry_by_group.items():
        ...
        for case in alis_cases:
            ...
            _, _, _, count = EXPECTED_ALIS_GRID
