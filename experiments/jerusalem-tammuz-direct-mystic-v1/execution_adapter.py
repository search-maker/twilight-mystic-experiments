...
class TammuzAdapterRefusal(RuntimeError):
    pass
...
def _load_module(name: str, path: Path):
    ...
    if spec is None or spec.loader is None:
        raise TammuzAdapterRefusal(f"cannot load reviewed adapter: {path}")
...
def load_generic_adapter():
    return _load_module("tammuz_generic_cross_geometry_execution_adapter", GENERIC_ADAPTER)


def load_elevation_adapter():
    return _load_module("tammuz_reviewed_ground_site_elevation_adapter", ELEVATION_ADAPTER)
...
def validate_rendered_aod550_binding(text: str, expected_aod550: Any) -> None:
    try:
        ...
        raise TammuzAdapterRefusal(f"invalid normalized AOD550: {expected_aod550}") from exc
    if abs(aod - EXPECTED_AOD550) > 1e-12:
        raise TammuzAdapterRefusal(f"Tammuz AOD550 changed: expected {EXPECTED_AOD550}, got {aod}")
    ...
    if directives != [expected]:
        raise TammuzAdapterRefusal(f"rendered AOD550 binding mismatch: expected exactly {expected!r}, got {directives!r}")


def validate_ground_site_rendering(base_text: str, corrected_text: str, observer_elevation_m: Any, site_altitude_km: float, atmosphere_grid_km: list[float]) -> None:
    try:
        elevation_m = float(observer_elevation_m)
    except (TypeError, ValueError) as exc:
        raise TammuzAdapterRefusal(f"invalid observer elevation: {observer_elevation_m}") from exc
    if abs(elevation_m - EXPECTED_OBSERVER_ELEVATION_M) > 1e-12:
        raise TammuzAdapterRefusal(f"Tammuz observer elevation changed: expected {EXPECTED_OBSERVER_ELEVATION_M}, got {elevation_m}")
    if base_text.splitlines().count("zout 0.800000") != 1:
        raise TammuzAdapterRefusal("unexpected pre-repair base elevation representation")
    if abs(site_altitude_km - 0.8) > 1e-12:
        raise TammuzAdapterRefusal(f"reviewed elevation adapter returned wrong site altitude: {site_altitude_km}")
    if not atmosphere_grid_km or abs(float(atmosphere_grid_km[0]) - 0.8) > 1e-12:
        raise TammuzAdapterRefusal("atm_z_grid does not start at the frozen 0.8 km site altitude")
    ...
    if len(grid_lines) != 1 or lines.count("zout 0.000000") != 1:
        raise TammuzAdapterRefusal("corrected input lacks exact atm_z_grid/local-surface zout representation")
    if any(line.startswith("zout 0.800000") for line in lines):
        raise TammuzAdapterRefusal("absolute zout 0.8 survived elevation repair")
    if any(line.startswith("altitude ") or line.startswith("mc_elevation_file ") for line in lines):
        raise TammuzAdapterRefusal("forbidden elevation shortcut emitted")
...
def prepare_case(proposal_path: Path, runtime_report_path: Path, case_id: str, data_dir: Path, repository_root: Path, output_dir: Path) -> dict[str, Any]:
    ...
    prepared = generic.prepare_case(proposal_path, runtime_report_path, case_id, data_dir, repository_root, output_dir)
    if prepared.get("batchId") != BATCH_ID:
        raise TammuzAdapterRefusal(f"wrong batchId: {prepared.get('batchId')}")
    inputs = prepared.get("inputs")
    if not isinstance(inputs, dict):
        raise TammuzAdapterRefusal("normalized inputs missing from generic prepared record")
    input_path = Path(prepared.get("inputPath", ""))
    if not input_path.is_file():
        raise TammuzAdapterRefusal(f"resolved input missing: {input_path}")
    ...
    if prepared.get("inputResolvedSha256") != base_input_sha256:
        raise TammuzAdapterRefusal("generic prepared input hash does not match rendered base input")

    corrected_text, site_altitude_km, atmosphere_grid_km = elevation.apply_ground_site_atm_z_grid(base_text, inputs.get("observerElevationM"))
    validate_ground_site_rendering(base_text, corrected_text, inputs.get("observerElevationM"), site_altitude_km, atmosphere_grid_km)
    ...
    prepared.update({
        ...
        "elevationRepairBoundary": "representation-only 800 m ground-site rendering already proven by consumed formal infrastructure smoke; frozen Tammuz science unchanged",
    })
    prepared_path = input_path.parent / "cross-geometry-prepared.json"
    prepared_path.write_text(json.dumps(prepared, indent=2, sort_keys=True, allow_nan=False) + "\n", encoding="utf-8", newline="\n")
    return prepared
