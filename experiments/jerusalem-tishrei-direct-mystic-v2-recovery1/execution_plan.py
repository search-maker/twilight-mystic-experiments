...
EXPECTED_GUARD_STATUS = "AUTHORIZED_V2_RECOVERY1_AFTER_CONSUMED_PREFLIGHT_FAILURE"
...
def build_plan(proposal_path: Path, guard_report_path: Path) -> dict[str, Any]:
    ...
    if guard.get("status") != EXPECTED_GUARD_STATUS or guard.get("laneId") != LANE_ID or guard.get("stageId") != SOURCE_STAGE_ID:
        raise PlanError("recovery1 authorization guard did not pass")
    if proposal.get("stageId") != SOURCE_STAGE_ID or proposal.get("batchId") != guard.get("batchId"):
        raise PlanError("source proposal and recovery1 guard do not match")
    ...
    return {
        ...
        "consumedSourceRunId": guard["consumedSourceRunId"],
        ...
        "cases": ordered,
        "matrix": matrix,
        "boundary": "recovery1 plan promotes the unchanged frozen 12-case source proposal only after authorization-1 is consumed and the full-ancestry recovery guard passes; no syntax or solver executed by planning",
    }
