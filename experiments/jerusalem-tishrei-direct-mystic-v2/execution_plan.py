...
def build_plan(proposal_path: Path, guard_report_path: Path) -> dict[str, Any]:
    ...
    if guard.get("status") != "AUTHORIZED_V2_AFTER_FORMAL_SMOKE_PASS" or guard.get("laneId") != LANE_ID or guard.get("stageId") != SOURCE_STAGE_ID:
        raise PlanError("v2 authorization guard did not pass")
    if proposal.get("stageId") != SOURCE_STAGE_ID or proposal.get("batchId") != guard.get("batchId"):
        raise PlanError("source proposal and v2 guard do not match")
    ...
    return {
        ...
        "cases": ordered,
        "matrix": matrix,
        "boundary": "v2 plan promotes the unchanged frozen 12-case source proposal only after v2 authorization and consumed formal infrastructure smoke; no syntax or solver executed by planning",
    }
