...
def load_manifest(path: Path):
    ...
    if m.get("row") != ROW or m.get("replicateSeedBases") != BASES:
        raise Failure("row or seed universe changed")
    if m.get("photonsPerDirectionPerCaseMethod") != PHOTONS:
        raise Failure("photon budget changed")
    if m.get("methods") != ["mc_vroom_off", "mc_vroom_on"]:
        raise Failure("method universe changed")
    ...
    if m.get("maximumSolverCalls") != 72 or m.get("maximumConfiguredPhotonHistories") != 144_000_000:
        raise Failure("maximum budget changed")
    analysis = m.get("analysis", {})
    if analysis.get("methodShiftToleranceMag") != 0.03:
        raise Failure("method-shift tolerance changed")
...
def mutate_estimator(text: str, method: str) -> str:
    old = "mc_vroom off"
    lines = text.splitlines()
    if lines.count(old) != 1:
    ...
    replacement = f"mc_vroom {method}"
    out = []
    for line in lines:
        if line == old:
            out.extend([replacement, "mc_escape on"])
        else:
            out.append(line)
    if out.count(replacement) != 1 or out.count("mc_escape on") != 1:
        raise Failure("estimator mutation failed")
    return "\n".join(out) + "\n"
...
def main():
    ...
    for method in METHODS:
        ...
        for case in ("baseline", "profile"):
            ...
            for direction in DIRECTIONS:
                ray = {
                    "thetaDeg": direction["thetaDeg"],
                    "relativeAzimuthDeg": direction["relativeAzimuthDeg"],
                }
                case_dir = out / "work" / method / case / direction["label"]
                if case == "baseline":
                ...
                records.append({
                    **direction,
                    **execute(base, uvspec, text, case_dir, direction["thetaDeg"], tables),
                })
            result[method][case] = records
    ...
    payload = {
        ...
        "photonsPerDirectionPerCaseMethod": PHOTONS,
        "commonRandomNumbersAcrossMethodsCasesDirections": True,
        "methods": {
            "off": "mc_vroom off + mc_escape on",
            "on": "mc_vroom on + mc_escape on",
        },
    ...
    print(json.dumps({"status": "COMPLETED", "replicate": a.replicate, "solverCalls": 12}, sort_keys=True))
