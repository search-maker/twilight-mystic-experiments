...
MANIFEST_ID='level-b-v2-densified58-fresh-validation-execution-manifest-v1'
STAGE_ID='LEVEL_B_V2_DENSIFIED58_FRESH_PROTECTED_VALIDATION_PREPARED_V1'
...
def validate_manifest(m):
    ...
    req((m.get('geometryCount'),m.get('caseCount'),m.get('configuredPhotonHistories'))==(6,24,960000000),'manifest accounting drift')
    req(all(c.get('role')=='protected-holdout' and c.get('executionStage')=='FRESH_PROTECTED_HOLDOUT_AFTER_DENSIFIED58_MODEL_FREEZE' and c.get('method')=='alis' and c.get('photonHistories')==40000000 for c in m.get('cases',[])),'case role/budget drift')
...
def render_case(manifest_path:Path,runtime_report_path:Path,case_id:str,data_dir:Path,repository_root:Path,case_dir:Path):
    ...
    base=module('stage1_adapter',repository_root/STAGE1_REL); runtime=load(runtime_report_path); base.validate_runtime(m,runtime); x=base.inputs(m,c,g)
    cross=module('v0070_cross',repository_root/base.CROSS_REL); elev=module('v0070_elev',repository_root/base.ELEV_REL)
