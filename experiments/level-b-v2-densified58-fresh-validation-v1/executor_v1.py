...
BRANCH_RE=re.compile(r'^dispatch/level-b-v2-densified58-fresh-validation-ordinal[1-9][0-9]*-v1$')
GRID_SHA='b5fae53c1cc88c7f3de6e3689bc25e4a36c54033d1d1bfd6169482f30cc5b477'
STAGE_ID='LEVEL_B_V2_DENSIFIED58_FRESH_PROTECTED_VALIDATION_EXECUTION_V1'
...
def parse_full_spectrum(path:Path):
    ...
    for line in path.read_text(encoding='utf-8',errors='strict').splitlines():
        ...
        row=[float(x) for x in p]; req(all(math.isfinite(x) for x in row) and all(x>=0 for x in row[1:]),'invalid spectrum value')
        toks.append(p[0]); rad.append(row[-1])
    req(len(toks)==8001 and toks[0]=='380.00000' and toks[-1]=='780.00000','raw spectrum grid/count drift')
    req(hashlib.sha256(('\n'.join(toks)+'\n').encode()).hexdigest()==GRID_SHA,'raw spectrum token grid drift')
    return [float(x) for x in toks],rad
...
def main()->int:
    ...
    base=module('stage1_executor',a.repository_root/STAGE1_REL); base.BRANCH_RE=BRANCH_RE; base.STAGE_ID=STAGE_ID; base.parse_full_spectrum=parse_full_spectrum
    try:
        r=base.execute_case(a.manifest,a.runtime_report,a.adapter,a.case_id,a.data_dir,a.repository_root,a.uvspec,a.output_root,a.timeout_seconds,a.allow_execution,a.expected_dispatch_branch)
        r.pop('contentSha256',None); r['stageId']=STAGE_ID; r['protectedHoldoutValueExposed']=True; r['ordinal22ValueExposed']=False; r['contentSha256']=base.canon(r)
        d=a.output_root/a.case_id; (d/'case-result.json').write_text(json.dumps(r,sort_keys=True,separators=(',',':'),allow_nan=False)+'\n',encoding='utf-8'); print(json.dumps(r,sort_keys=True)); return 0
    except Exception as e:
        exposed=(a.output_root/a.case_id/'mc.rad.spc').is_file(); f={'schemaVersion':1,'stageId':STAGE_ID,'status':'FAILED_OR_REFUSED_TERMINAL_ATTEMPT1','caseId':a.case_id,'workflowRunAttempt':int(os.getenv('GITHUB_RUN_ATTEMPT') or 0),'retryPerformed':False,'resumePerformed':False,'githubRerun':False,'reason':str(e),'protectedHoldoutValueExposed':exposed,'ordinal22ValueExposed':False}; f['contentSha256']=base.canon(f)
        try:
