...
The analysis compares held-out results with the frozen precise counterpart, requires held-out relative standard error of the mean no greater than 8 percent, retains the existing broad spectral screening checks, and emits one audited computational reference dataset combining all carried screening-agreement geometries with every held-out geometry that passes.
