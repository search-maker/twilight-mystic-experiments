...
def build_plan(proposal_path: Path, guard_report_path: Path) -> dict[str, Any]:
    ...
    if proposal.get("stageId") != STAGE_ID or proposal.get("batchId") != guard.get("batchId"):
        raise PlanRefusal("proposal and guard do not match")
    ...
    return {
        ...
        "cases": ordered,
        "matrix": matrix,
        "boundary": "plan promotes one exact proposal only after one-purpose authorization; no syntax or solver executed",
    }
