...
def audit(plan_path: Path, cases_root: Path, aggregate_dir: Path, output_path: Path) -> tuple[dict[str, Any], bool]:
    ...
    for group_id, expected_cases in groups.items():
        ...
        values = [float(records[case_id]["selectedPhotopicContributionCdM2"]) for case_id in expected_case_ids]
        expected_geometry_classification = (
            "ADAPTIVE_CONTINUATION_REQUIRED" if expected_zero_ids else "BATCH_NUMERICALLY_COMPLETE"
        )
