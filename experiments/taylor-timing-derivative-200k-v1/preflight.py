...
ROWS=[22,26]; REPLICATES=[1,2,3,4,5,6]; PHOTONS=200_000
SEED_BASE={1:973_000_000,2:974_000_000,3:975_000_000,4:976_000_000,5:977_000_000,6:978_000_000}
REFERENCE_PHOTONS=50_000; REFERENCE_SEED_BASE=941_000_000
...
def main():
    ...
    for row in ROWS:
        ...
        for rep in REPLICATES:
            for ray in rays:
                ...
                ref=base.render(data,atm,Path('/tmp/ref')/f'r{row}'/f'p{rep}'/f'x{idx}',obs,ray,aod,REFERENCE_PHOTONS,refseed)
                new=base.render(data,atm,Path('/tmp/new')/f'r{row}'/f'p{rep}'/f'x{idx}',obs,ray,aod,PHOTONS,newseed)
                if normalize(ref)!=normalize(new): raise RuntimeError(f'physical input drift row={row} rep={rep} ray={idx}')
    ...
    if len(audit)!=768 or len(seeds)!=768 or len(set(seeds))!=768: raise RuntimeError('new timing-neighbor universe mismatch')
    consumed={base0+row*1000+ray for base0 in range(955_000_000,973_000_000,1_000_000) for row in ROWS for ray in range(1,65)}
    if set(seeds)&consumed: raise RuntimeError('new timing-neighbor seeds overlap consumed namespaces')
    a.output.mkdir(parents=True,exist_ok=False); result={'schemaVersion':1,'stageId':'taylor-timing-derivative-200k-v1','status':'DRY_TIMING_NEIGHBOR_IDENTITY_PASS','rows':ROWS,'replicates':REPLICATES,'photonsPerRay':PHOTONS,'uniqueSeedCount':len(set(seeds)),'auditCount':len(audit),'solverCallsIfAuthorized':768,'configuredPhotonHistoriesIfAuthorized':153_600_000,'audit':audit}; (a.output/'preflight.json').write_text(json.dumps(result,indent=2,sort_keys=True,allow_nan=False)+'\n'); print(json.dumps({k:result[k] for k in ('status','uniqueSeedCount','auditCount','solverCallsIfAuthorized','configuredPhotonHistoriesIfAuthorized')},sort_keys=True))
