...
def apply_ground_site_atm_z_grid(
    ...
    elevation = _module(elevation_path, "wave1_reviewed_elevation_adapter")
    try:
        corrected, site_altitude_km, atmosphere_grid_km = (
            elevation.apply_ground_site_atm_z_grid(
                rendered,
        ...
        raise AdapterRefusal(
            f"reviewed atm_z_grid correction refused input: {exc}"
        ) from exc
...
def prepare_case(
    ...
    if any(
        runtime.get(key) != manifest["runtime"].get(key)
        for key in required_runtime
    ):
        raise AdapterRefusal("runtime hash drift")
    matches = [
        case for case in manifest["cases"] if case.get("caseId") == case_id
    ]
    if len(matches) != 1:
        raise AdapterRefusal("case must occur exactly once")
    case = matches[0]
    if case.get("block") not in (3, 4) or case.get("role") not in (
        "surrogate-training",
        "internal-holdout",
    ):
    ...
    base = _module(base_path, "wave1_base_adapter")
    ...
    prepared = {
        ...
        "elevationAdapterRawSha256": raw_sha256(
            repository_root / ELEVATION_ADAPTER
        ),
        "inputResolvedSha256": hashlib.sha256(corrected.encode()).hexdigest(),
        "inputPath": str(input_path),
        "fittingSurfaceExposed": False,
        "boundary": (
            "input rendering only with reviewed atm_z_grid elevated-site "
            "representation; no syntax check or solver execution"
        ),
    }
