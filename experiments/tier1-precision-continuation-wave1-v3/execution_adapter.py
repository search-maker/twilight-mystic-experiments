...
def _base(repository_root: Path):
    path = repository_root / BASE_ADAPTER
    spec = importlib.util.spec_from_file_location("wave1_v3_reviewed_base_adapter", path)
    if spec is None or spec.loader is None:
        raise AdapterRefusal("reviewed base execution adapter unavailable")
...
def prepare_case(
    ...
    return _base(repository_root).prepare_case(
        manifest_path,
        runtime_report_path,
        case_id,
        data_dir,
