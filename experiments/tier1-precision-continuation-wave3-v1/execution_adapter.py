...
def apply_ground_site_atm_z_grid(
    ...
    elevation = _module(elevation_path, "wave3_reviewed_elevation_adapter")
    try:
...
def prepare_case(
    ...
    matches = [case for case in cases if case.get("caseId") == case_id]
    if len(matches) != 1:
    ...
    base = _module(base_path, "wave3_base_adapter")
    ...
    prepared = {
        ...
        "boundary": "input rendering only for dynamic b7-b8 active set; no syntax check or solver execution",
    }
