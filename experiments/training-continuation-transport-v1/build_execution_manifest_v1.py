...
def build(repo:Path,variant:str,prereg_path:Path,contract_path:Path,analysis_path:Path)->dict:
    ...
    for x in cases:
        ...
        rows.append({'caseId':cid,'geometryId':x['geometryId'],'method':x['method'],'importanceCenterNm':float(x['importanceCenterNm']),'block':int(x.get('trainingAcquisitionBlock',x.get('comparisonBlock'))),'seed':int(x['seed']),'photonHistories':int(x['photonHistories']),'templatePath':str(path.relative_to(repo)),'templateSha256':x['templateSha256'],'templatePhysicalFingerprintSha256':x['templatePhysicalFingerprintSha256']})
...
def main():
    ...
    try: m=build(z.repository_root.resolve(),z.variant,z.preregistration,z.contract,z.analysis_contract); z.output.write_text(json.dumps(m,indent=2,sort_keys=True)+'\n'); print(json.dumps({'status':'PASSED','manifestSha256':m['manifestSha256'],'caseCount':m['caseCount']})); return 0
