...
PREAUTH_BRANCH = "review/aerosol-vertical-profile-sensitivity-v2-postconsumption-recovery3-preauthorization-v1"
BASE_MAIN = "384f94c02b892768a9dde6fe17e69a384112ef73"
CONSUMED_ORDINALS = (41, 42, 43)
LATEST_CONSUMED_ORDINAL = 43
...
def bound_scanner():
    ...
    return load_module("avps_v2_recovery3_preauth_global_scanner", GENERIC_SCANNER)
...
def bound_ordinal_module():
    ...
    freshness = load_module("avps_v2_recovery3_bound_r8_freshness", freshness_path)
    previous = sys.modules.get("freshness")
    sys.modules["freshness"] = freshness
    try:
        ordinal = load_module("avps_v2_recovery3_bound_r8_ordinal", ordinal_path)
    finally:
        if previous is None:
...
def recovery_ledger() -> dict[str, Any]:
    ...
    ledger = load_module("avps_v2_recovery3_preauth_seed_ledger", RECOVERY_LEDGER).validate_ledger()
    expected = {
