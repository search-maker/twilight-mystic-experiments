...
PREAUTH_BRANCH = "review/aerosol-vertical-profile-sensitivity-v2-postconsumption-recovery3-preauthorization-v2"
...
def bound_scanner():
    ...
    return load_module("avps_v2_recovery3_preauth_v2_global_scanner", GENERIC_SCANNER)
...
def bound_ordinal_module():
    ...
    freshness = load_module("avps_v2_recovery3_preauth_v2_freshness", freshness_path)
    previous = sys.modules.get("freshness")
    sys.modules["freshness"] = freshness
    try:
        ordinal = load_module("avps_v2_recovery3_preauth_v2_ordinal", ordinal_path)
    finally:
        if previous is None:
...
def recovery_ledger() -> dict[str, Any]:
    ...
    ledger = load_module("avps_v2_recovery3_preauth_v2_seed_ledger", RECOVERY_LEDGER).validate_ledger()
    expected = {
...
def build_report(
    ...
    report = {
        "schemaVersion": 1,
        "stageId": f"{STAGE}-preauthorization-v2",
...
def final_verify(
    ...
    for key in (
        "consumedFloorScientificOrdinal",
