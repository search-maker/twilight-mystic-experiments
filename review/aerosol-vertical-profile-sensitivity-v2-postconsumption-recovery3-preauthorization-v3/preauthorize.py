...
PREAUTH_BRANCH = "review/aerosol-vertical-profile-sensitivity-v2-postconsumption-recovery3-preauthorization-v3"
BASE_MAIN = "384f94c02b892768a9dde6fe17e69a384112ef73"
CONSUMED_ORDINALS = (41, 42, 43)
CONSUMED_FLOOR = 43
...
RECOVERY_LEDGER = ROOT / "review/aerosol-vertical-profile-sensitivity-v2-postconsumption-recovery3-seed-freshness-v1/seed_ledger.py"
RECOVERY_LEDGER_BLOB = "a4fc0b95c3627a310c0c17a1ae8b89701511b3b8"
...
def bound_scanner():
    ...
    return load_module("avps_v2_recovery3_preauth_v3_global_scanner", GENERIC_SCANNER)
...
def bound_ordinal_module():
    ...
    freshness = load_module("avps_v2_recovery3_preauth_v3_freshness", freshness_path)
    previous = sys.modules.get("freshness")
    sys.modules["freshness"] = freshness
    try:
        ordinal = load_module("avps_v2_recovery3_preauth_v3_ordinal", ordinal_path)
    finally:
        if previous is None:
...
def recovery_ledger() -> dict[str, Any]:
    if git_blob_sha1(RECOVERY_LEDGER) != RECOVERY_LEDGER_BLOB:
        raise Refusal("recovery3 candidate-ledger byte drift")
    ledger = load_module("avps_v2_recovery3_preauth_v3_seed_ledger", RECOVERY_LEDGER).validate_ledger()
    expected = {
    ...
    for key, value in expected.items():
        if ledger.get(key) != value:
            raise Refusal(f"recovery3 candidate ledger drift: {key}")
    if len(set(int(x) for x in ledger.get("candidateSeeds", []))) != EXPECTED_SEED_COUNT:
        raise Refusal("recovery3 candidate seed uniqueness/cardinality drift")
    for key in ("candidateSeedsAppliedToCases", "scientificOrdinalAllocated", "authorizationCreated", "dispatchCreated", "solverExecutionAuthorized", "resultOpeningAuthorized", "productionAuthorized"):
        if ledger.get(key) is not False:
            raise Refusal(f"recovery3 candidate ledger crossed boundary: {key}")
    return ledger
...
def ordinal_surface(payload: dict[str, Any], current_run_id: int | None) -> dict[str, Any]:
    ...
    if occupied_max < CONSUMED_FLOOR:
        raise Refusal("global ordinal surface regressed below consumed ordinal 43")
    next_ordinal = occupied_max + 1
    if next_ordinal <= CONSUMED_FLOOR:
        raise Refusal("derived successor ordinal is not newer than consumed ordinal 43")
    ...
    if auth_branch in branch_names or dispatch_branch in branch_names:
        raise Refusal("proposed recovery3 authorization/dispatch branch already exists")
    marker_prefix = f"ORDINAL{next_ordinal}_"
    if any(str(row.get("body") or "").strip().upper().startswith(marker_prefix) for row in payload.get("issue60Comments", [])):
    ...
    return {
        ...
        "consumedFloorScientificOrdinal": CONSUMED_FLOOR,
...
def build_report(payload: dict[str, Any], seed_global_report: dict[str, Any], expected_head: str, current_run_id: int | None) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    ...
    report = {
        "schemaVersion": 1,
        "stageId": f"{STAGE}-preauthorization-v3",
        ...
        **ordinal,
        ...
        "overlapWithConsumedOrdinal43SeedCount": int(ledger["overlapWithConsumedOrdinal43SeedCount"]),
...
def final_verify(repository: str, token: str, expected_head: str, current_run_id: int | None, report: dict[str, Any]) -> None:
    ...
    for key in ("consumedFloorScientificOrdinal", "occupiedMaxScientificOrdinal", "nextAvailableScientificOrdinal", "authorizationBranch", "dispatchBranch"):
...
def main() -> int:
    ...
    if args.command == "build":
        ...
        report, observations = build_report(payload, json.loads(args.seed_global_report.read_text()), args.expected_head, args.current_run_id)
        args.output.write_text(json.dumps(report, indent=2, sort_keys=True) + "\n")
        args.observations_output.write_text(json.dumps(observations, indent=2, sort_keys=True) + "\n")
        return 0
    report = json.loads(args.report.read_text())
    if report.get("status") != "PASS_POSTCONSUMPTION_RECOVERY3_PREAUTHORIZATION_SURFACE_CLEAN_NOT_ALLOCATED":
