...
CONTROL_BRANCH = "review/avps-v2-postconsumption-successor-authorization-control-replacement-v1-20260909"
...
SCANNER_AUDIT_MODE = "authorization-recheck"
...
def repo_url(repository: str, suffix: str) -> str:
    ...
    return root if not normalized else f"{root}/{normalized}"


def repo_url_fixture() -> dict[str, str]:
    root = "https://api.github.com/repos/owner/repo"
    require(repo_url("owner/repo", "") == root, "repository-root REST URL gained trailing slash")
    require(repo_url("owner/repo", "/") == root, "repository-root slash normalization drift")
    require(repo_url("owner/repo", "branches/main") == f"{root}/branches/main", "nonempty repository suffix drift")
    require(repo_url("owner/repo", "/branches/main") == f"{root}/branches/main", "leading-slash suffix normalization drift")
    return {
        "root": repo_url("owner/repo", ""),
        "rootSlash": repo_url("owner/repo", "/"),
        "suffix": repo_url("owner/repo", "branches/main"),
        "leadingSlashSuffix": repo_url("owner/repo", "/branches/main"),
    }


def require_bound_scanner_mode(scanner) -> list[str]:
    modes = getattr(scanner, "AUDIT_MODES", None)
    require(isinstance(modes, (set, frozenset)) and SCANNER_AUDIT_MODE in modes, "bound scanner does not expose authorization-recheck audit mode")
    require("authorization-control" not in modes, "bound scanner unexpectedly exposes unsupported authorization-control synonym")
...
def stress(args) -> None:
    ...
    try:
        ...
        scanner_modes = require_bound_scanner_mode(scanner_wrapper.mod)
    ...
    write_json(evidence / "repository-root-url-fixture.json", {"urls": url_fixture, "liveRepositoryFullName": static["repo"].get("full_name"), "liveDefaultBranch": static["repo"].get("default_branch")})
    write_json(evidence / "bound-scanner-mode-fixture.json", {"requiredMode": SCANNER_AUDIT_MODE, "exposedModes": scanner_modes, "unsupportedAuthorizationControlAbsent": True})
    ...
    write_json(evidence / "stress-result.json", {
        ...
        "repositoryRootUrlFixturePassed": True,
        "liveRepositoryIdentityReadbackPassed": True,
        "boundScannerAuthorizationRecheckModeAsserted": True,
...
def control(args) -> None:
    ...
    try:
        ...
        require_bound_scanner_mode(scanner)
        ...
        context, stable_sha, fence, post_counts = scanner.collect_stable(
            args.repository, 60, args.token, args.run_id, seeds, SCANNER_AUDIT_MODE
        )
