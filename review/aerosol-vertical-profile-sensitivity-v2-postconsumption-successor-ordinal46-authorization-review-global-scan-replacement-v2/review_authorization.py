...
RBR='review/avps-v2-postconsumption-successor-ordinal46-authorization-review-global-scan-replacement-v2-20260910'; STAGE='AVPS_V2_POSTCONSUMPTION_SUCCESSOR_ORDINAL46_AUTHORIZATION_REVIEW_GLOBAL_SCAN_REPLACEMENT_V2'
WF='.github/workflows/avps-v2-postconsumption-successor-ordinal46-authorization-review-global-scan-replacement-v2.yml'; SCRIPT='review/aerosol-vertical-profile-sensitivity-v2-postconsumption-successor-ordinal46-authorization-review-global-scan-replacement-v2/review_authorization.py'; ART='avps-v2-postconsumption-successor-ordinal46-authorization-review-global-scan-replacement-v2-proof'
...
def static(c,a):
    ...
    local_tree=out('git','rev-parse','HEAD^{tree}',cwd=INFRA); api_commit=c.core.req_json(c.core.repo_url(a.repo,f'git/commits/{a.rh}'),a.tok); req(str((api_commit.get('tree') or {}).get('sha') or '')==local_tree,'reviewer tree API/local drift')
    ...
    req(shutil.which('uvspec') is None,'uvspec present in zero-runtime review'); return paths,local_tree
...
def review(a):
    req(not out('git','status','--porcelain','--untracked-files=no'),'tracked subject workspace dirty'); req(not out('git','status','--porcelain'),'subject workspace dirty'); c=ctrl(); req(not out('git','status','--porcelain',cwd=INFRA),'reviewer workspace dirty'); a.ev.mkdir(parents=True,exist_ok=False)
    paths,reviewer_tree=static(c,a); pre=wq(c,a.repo,a.tok,a.rh,a.rpr); subject=(ROOT/AUTH).read_bytes(); crec=control_artifact(c,a,subject); source_api=c.core.verify_source_api(a.repo,a.tok)
