...
def build(
    ...
    seed_mod = load_module("avps_v2_seed_ledger_for_freshness_proof", stage_dir / "seed_ledger.py")
    ...
    if ledger.get("trackedCandidateSeedLedger") is not False or ledger.get("candidateSeedsAppliedToCases") is not False:
        raise Refusal("candidate seed artifact boundary drift")
    ...
    if global_report.get("repositoryHeadExpected") != expected_head or global_report.get("auditedBranchHeadShaObserved") != expected_head:
        raise Refusal("repository-global expected/observed head drift")
    if global_report.get("priorReviewProofArtifactCount") != 0 or global_report.get("reviewProofIdentityFresh") is not True:
        raise Refusal("v2 candidate seed review-proof identity already existed")
