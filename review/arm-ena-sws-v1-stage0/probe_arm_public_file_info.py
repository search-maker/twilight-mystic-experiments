...
UA = "Mozilla/5.0 ARM-ENA-SWS-V1-public-file-info-probe/3"
TEST_DS = "enaswsC1.b1"
TEST_DATE = "20170616"
TEST_PREFIX = f"{TEST_DS}.{TEST_DATE}"
...
PROBES = [
    ...
    ("metadata_datastream_keyword", qurl(EMETA, f'datastream.keyword:"{TEST_DS}"', 10), "GET", None),
    ("metadata_datastream_text", qurl(EMETA, f'datastream_text:"{TEST_DS}"', 10), "GET", None),
    ("metadata_concat_datastream", qurl(EMETA, f'concat_datastream:"{TEST_DS}"', 10), "GET", None),
    ("metadata_identifier", qurl(EMETA, f'identifier:"{TEST_DS}"', 10), "GET", None),
    ("fileinfo_datastream", qurl(EFILE, f'datastream:"{TEST_DS}"', 20), "GET", None),
    ("fileinfo_datastream_keyword", qurl(EFILE, f'datastream.keyword:"{TEST_DS}"', 20), "GET", None),
    ("fileinfo_file_name", qurl(EFILE, f'file_name:{TEST_PREFIX}*', 20), "GET", None),
    ("fileinfo_file_name_keyword", qurl(EFILE, f'file_name.keyword:{TEST_PREFIX}*', 20), "GET", None),
    ("fileinfo_free_text", qurl(EFILE, TEST_DS, 10), "GET", None),
    ("fileinfo_match_phrase_post", EFILE, "POST", {"size": 20, "query": {"match_phrase": {"datastream": TEST_DS}}}),
    ("fileinfo_term_keyword_post", EFILE, "POST", {"size": 20, "query": {"term": {"datastream.keyword": TEST_DS}}}),
    ("fileinfo_prefix_post", EFILE, "POST", {"size": 20, "query": {"prefix": {"file_name": TEST_PREFIX}}}),
    ("fileinfo_prefix_keyword_post", EFILE, "POST", {"size": 20, "query": {"prefix": {"file_name.keyword": TEST_PREFIX}}}),
]
...
def summarize(obj):
    if not isinstance(obj, dict) or not isinstance(obj.get("hits"), dict):
        return None
    h = obj["hits"]
        ...
        rows.append(
            {
                ...
                "safe_source": safe_source(src),
            }
        )
    target_rows = [
        row
        for row in rows
        if row.get("safe_source", {}).get("datastream") == TEST_DS
        or str(row.get("safe_source", {}).get("file_name", "")).startswith(TEST_PREFIX)
    ]
    return {
        "total": h.get("total"),
        "hit_count_returned": len(hits),
        "target_match_count_returned": len(target_rows),
        "hits": rows,
    }
...
def main() -> int:
        ...
        rows.append(
            {
                ...
                "json_parse_ok": rec.get("json_parse_ok"),
                "result": summarize(rec.get("json")),
            }
        )
    result = {
        "schema": 3,
        "protocol": "ARM_ENA_SWS_V1_ANONYMOUS_SEARCH_INDEX_METADATA_PROBE",
        "test_datastream": TEST_DS,
        "test_date": TEST_DATE,
        "probe_results": rows,
        "anonymous_search_endpoint_responds_json": any(
            x.get("status") == "OK" and x.get("json_parse_ok") for x in rows
        ),
    ...
    (OUT / "arm_public_file_info_probe_raw.json").write_text(
        json.dumps(raw, indent=2, sort_keys=True) + "\n", encoding="utf-8"
    )
    (OUT / "arm_public_file_info_probe_summary.json").write_text(
