#!/usr/bin/env python3
"""Run the ENA/SWS E0 stream collector against the frozen 906-event universe.

The canonical universe SHA-256 is frozen in issue #60 comment 5487782757.
This wrapper either verifies a supplied CSV byte-for-byte or regenerates the
same universe with generate_ena_event_universe_fast.py and verifies its SHA
before any ARM query is possible. It then injects only those frozen Event rows
into the already-audited streaming collector.
"""
...
def generate_verified_csv(generator_path: Path, e0, destination: Path):
    gen = load_module("ena_fast_generator", generator_path)
...
def main() -> int:
    ...
    if known.event_universe is not None:
        ...
        temp_ctx = tempfile.TemporaryDirectory(prefix="ena_frozen_universe_")
    ...
    collector = load_module("ena_sws_stream_collector", collector_path)
    # The collector still hashes the on-disk E0 auditor source for provenance,
    # but receives the already-loaded module whose build_events is replaced by
    # the verified frozen rows. No selection or data-dependent mutation occurs.
