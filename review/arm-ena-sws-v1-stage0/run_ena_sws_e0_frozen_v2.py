#!/usr/bin/env python3
"""Run the ENA/SWS E0 stream collector using the prospective frozen-contract v2 auditor.

The canonical 906-event universe SHA-256 is frozen in issue #60 comment
5487782757.  This wrapper preserves the historical result-blind universe and
streaming transport but routes structural disposition through
`audit_ena_sws_e0_v2.py`, which aligns usable-anchor counts with the original
Stage-0 preregistration (#60 5487647692).  Protected photometric values remain
sealed throughout E0.
"""
...
def generate_verified_csv(generator_path: Path, e0, destination: Path):
    gen = load_module("ena_fast_generator_v2", generator_path)
...
def main() -> int:
    ...
    collector_path = here / "stream_ena_sws_e0_from_arm_live.py"
    generator_path = here / "generate_ena_event_universe_fast.py"
    e0 = load_module("ena_sws_e0_frozen_v2", e0_path)
    ...
    if known.event_universe is not None:
        ...
        temp_ctx = tempfile.TemporaryDirectory(prefix="ena_frozen_universe_v2_")
    ...
    collector = load_module("ena_sws_stream_collector_v2", collector_path)
    # The collector hashes the v2 auditor source for provenance and receives the
    # already-loaded v2 module whose build_events is replaced by the immutable
    # frozen rows.  No selection or data-dependent mutation occurs.
    e0.build_events = lambda: list(events)
    collector.load_e0 = lambda _path: e0
