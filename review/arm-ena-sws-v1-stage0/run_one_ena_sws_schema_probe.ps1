# Result-blind one-event ARM ENA/SWS E0 schema probe for Windows.
...
$Runner = Join-Path $Here "run_ena_sws_e0_frozen.py"
$Venv = Join-Path $Here ".venv-ena-sws-e0"
$VenvPython = Join-Path $Venv "Scripts\python.exe"

if (-not (Test-Path -LiteralPath $Req)) { throw "Missing requirements file: $Req" }
if (-not (Test-Path -LiteralPath $Runner)) { throw "Missing frozen E0 runner: $Runner" }
...
$OutDir = Join-Path $OutputParent ("ARM_ENA_SWS_E0_SCHEMA_PROBE_" + $timestamp)
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "ENA/SWS schema probe failed with exit code $LASTEXITCODE." }
} finally {
...
$receipt = [ordered]@{
    schema = 1
    purpose = "ARM_ENA_SWS_V1_E0_ONE_EVENT_SCHEMA_PROBE"
...
Write-Host ""
Write-Host "ENA/SWS one-event schema probe complete."
