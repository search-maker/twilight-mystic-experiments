...
def make_arscl(p,positive=False,missing=False):
...
def make_mfrsr(p,vals,qbad=False):
    with netCDF4.Dataset(p,'w') as ds:
        ...
        w=ds.createVariable('filter2_wavelength','f4'); w.long_name='nominal wavelength filter2'; w.assignValue(501.5)
...
def main():
    with tempfile.TemporaryDirectory() as td:
        r=Path(td)
        make_arscl(r/'a_clear.nc'); make_arscl(r/'a_cloud.nc',positive=True); make_arscl(r/'a_missing.nc',missing=True)
        ac=G.analyze_arscl(r/'a_clear.nc',START,END); ap=G.analyze_arscl(r/'a_cloud.nc',START,END); am=G.analyze_arscl(r/'a_missing.nc',START,END)
        assert ac['clear_evidence'] and not ac['positive']; assert ap['positive']; assert not am['clear_evidence']
        ...
        vals=np.full(20,0.08); make_mfrsr(r/'m_pass.nc',vals); m=G.analyze_mfrsr(r/'m_pass.nc',BASE,BASE+1000,0.07); assert m['pass']
        ...
        print('PASS ENA native gate synthetic contracts')
