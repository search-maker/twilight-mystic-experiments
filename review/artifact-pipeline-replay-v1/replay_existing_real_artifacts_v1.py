...
def main():
 ...
 try:
  ...
  fps=[inspect(paths[(c['geometryId'],int(c['block']))],b,c) for c in cases]; fps.sort(key=lambda e:e['caseId']); rec=b.aggregate(cases); zeros=sorted(c['caseId'] for c in cases if c['rawAllZero']); req(len(rec)==39 and len(zeros)==14,'aggregate/zero count')
