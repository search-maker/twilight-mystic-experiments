...
def run_read_only_gh_api(
    ...
    *,
    ...
    sleep_seconds: tuple[int, ...] = BACKOFF_SECONDS,
) -> dict:
    if max_attempts != MAX_ATTEMPTS:
        raise SystemExit(f"Recovery v4 freezes max_attempts={MAX_ATTEMPTS}")
    if tuple(sleep_seconds) != BACKOFF_SECONDS:
        raise SystemExit("Recovery v4 freezes deterministic backoff schedule")
