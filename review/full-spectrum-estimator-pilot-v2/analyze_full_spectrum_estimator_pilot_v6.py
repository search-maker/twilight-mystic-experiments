...
ANALYSIS_ID='public-tier1-full-spectrum-estimator-pilot-analysis-v6'
ACQUISITION_PROTOCOL_SHA='7ca0923204452ab203249dfd060dd5fef5465c48a20ba529c0a20748e0152434'
ANALYSIS_PROTOCOL_SHA='ad847ecb7f46629787148c572fe0e6d6d26c7eda12837d74de00b28abb64de6f'
...
def method_stats(rows:list[dict[str,Any]])->dict[str,Any]:
 ...
 primary_zero_channels=[name for name in PRIMARY if channels[name]['zeroHitPresent']]
 primary_zero_case_count=sum(1 for r in ordered if any(float(r['channels'][name])==0.0 for name in PRIMARY))
 all_primary_zero_case_count=sum(1 for r in ordered if all(float(r['channels'][name])==0.0 for name in PRIMARY))
 return {'channels':channels,'derivedScotopicPhotopicRatio':{'values':spr,'mean':sum(finite)/len(finite) if finite else None,'maxMinRatio':(max(finite)/min(finite) if len(finite)==2 and min(finite)>0 else None)},'primaryZeroHitChannels':primary_zero_channels,'anyPrimaryChannelZeroHit':bool(primary_zero_channels),'primaryZeroHitCaseCount':primary_zero_case_count,'allPrimaryChannelsZeroHitCaseCount':all_primary_zero_case_count,'screeningBlockCount':2,'inferentialVarianceClaim':False}
...
def same_n_variance_screen(fresh:dict[str,Any],historical:dict[str,Any],problem_channels:list[str])->dict[str,Any]:
 per={}; applicable=[]; non_problem=[]
 for name in PRIMARY:
  fr=fresh['channels'][name]['descriptiveTwoBlockRsem']; hr=historical[name]['firstTwoScreening']['descriptiveTwoBlockRsem']
  is_problem=name in problem_channels
  if is_problem:
   applicable.append(name)
   if fr is None or hr is None or hr<=0:
    per[name]={'role':'historical-problem','computable':False,'freshTwoBlockRsem':fr,'historicalFirstTwoBlockRsem':hr,'varianceProxyRatio':None,'maximumAllowedVarianceProxyRatio':VARIANCE_PROXY_MAX,'passed':False}
   else:
    ratio=(fr/hr)**2
    per[name]={'role':'historical-problem','computable':True,'freshTwoBlockRsem':fr,'historicalFirstTwoBlockRsem':hr,'varianceProxyRatio':ratio,'maximumAllowedVarianceProxyRatio':VARIANCE_PROXY_MAX,'passed':ratio<=VARIANCE_PROXY_MAX}
  else:
   non_problem.append(name)
   threshold=max(0.08,hr) if hr is not None else 0.08
   ok=fr is not None and fr<=threshold
   ratio=(fr/hr)**2 if fr is not None and hr is not None and hr>0 else None
   per[name]={'role':'historical-non-problem','computable':fr is not None,'freshTwoBlockRsem':fr,'historicalFirstTwoBlockRsem':hr,'varianceProxyRatio':ratio,'maximumAllowedFreshTwoBlockRsem':threshold,'passed':ok}
 problem_usable=bool(applicable) and all(per[n]['computable'] for n in applicable)
 problem_pass=problem_usable and all(per[n]['passed'] is True for n in applicable)
 non_problem_pass=all(per[n]['passed'] is True for n in non_problem)
 no_fresh_primary_zero=not fresh.get('anyPrimaryChannelZeroHit',False)
 passed=problem_pass and non_problem_pass and no_fresh_primary_zero
 return {'formula':'problem channels: (freshTwoBlockRsem / historicalFirstTwoBlockRsem)^2; non-problem channels: freshTwoBlockRsem <= max(0.08, historicalFirstTwoBlockRsem)','sameBlockCount':2,'samePerBlockPhotonHistoriesRequired':True,'historicalProblemChannels':applicable,'historicalNonProblemChannels':non_problem,'perChannel':per,'allProblemChannelsComputable':problem_usable,'allProblemChannelsPass':problem_pass,'allNonProblemChannelsPassNonDegradation':non_problem_pass,'freshPrimaryChannelZeroHit':not no_fresh_primary_zero,'passed':passed,'inferentialClaim':False}
...
def validate_analysis_protocol(ap:dict[str,Any], acquisition:dict[str,Any], admission:dict[str,Any])->dict[str,Any]:
 ...
 for frozen in rows:
  ...
  for name in PRIMARY:
   ...
   channels[name]={'firstTwoValues':vals[:2],'firstTwoMean':st['mean'],'firstTwoSampleStd':st['sampleStd'],'firstTwoDescriptiveRsem':st['descriptiveTwoBlockRsem'],'firstTwoZeroHitCount':sum(1 for x in vals[:2] if x==0.0),'fullHistoryBlockCount':int(c['blockCount']),'fullHistoryMean':sum(vals)/len(vals),'fullHistoryRsem':full,'fullHistoryZeroHitCount':z}
...
def analyze(acquisition:dict[str,Any],analysis_protocol:dict[str,Any],admission:dict[str,Any],evidence:dict[str,Any])->dict[str,Any]:
 ...
 for sel in acquisition['selectedGeometries']:
  ...
  if vstat:
   ...
   if vstat['anyPrimaryChannelZeroHit']: cls='REFERENCE_RARE_EVENT_UNRESOLVED'
  ...
  for center in sorted({r['importanceCenterNm'] for r in gro if r['method']=='alis-alt-importance'}):
   ...
   if astat['anyPrimaryChannelZeroHit']:
    cls='RARE_EVENT_UNRESOLVED' if historical_rare else 'NO_CLEAR_SCREENING_GAIN'
   elif historical_rare:
    if vstat and not vstat['anyPrimaryChannelZeroHit'] and refcomp and any_ratio_fail(refcomp): cls='GROSS_METHOD_DISAGREEMENT'
    elif vstat and not vstat['anyPrimaryChannelZeroHit'] and refcomp and all_ratio_pass(refcomp): cls='RARE_EVENT_TWO_METHOD_FINITE_SCREENING_REQUIRES_CONFIRMATION'
 ...
 out={'schemaVersion':1,'analysisId':ANALYSIS_ID,'status':'PILOT_SCREENING_ANALYZED_NO_AUTOMATIC_ESTIMATOR_SELECTION','acquisitionProtocolSha256':ACQUISITION_PROTOCOL_SHA,'screeningAnalysisProtocolSha256':ANALYSIS_PROTOCOL_SHA,'executionManifestSha256':EXEC_SHA,'sourceTrainingDatasetSha256':SOURCE_DATASET_SHA,'sourceAdmissionReportSha256':ADMISSION_REPORT_SHA,'normalizedEvidenceSha256':es,'geometryCount':len(reports),'classificationCounts':counts,'geometryReports':reports,'statisticalInferenceClaim':False,'twoBlockRsemFinalPrecisionClaim':False,'fullAdaptiveHistoricalRsemUsedForVarianceGainThreshold':False,'freshPrimaryChannelZeroPolicyEnforced':True,'nonProblemChannelVarianceNonDegradationGuardEnforced':True,'automaticGlobalEstimatorSelection':False,'continuationAuthorized':False,'fittingAuthorized':False,'holdoutOpeningAuthorized':False,'productionAuthorization':False}
