...
def prepare_inso_alias(data_dir: Path) -> dict:
    ...
    if target.exists():
        raise CapabilityInputError(f"INSO resolver alias unexpectedly preexists: {target}")
    target.parent.mkdir(parents=True, exist_ok=True)
    source_bytes = source.read_bytes()
    target.write_bytes(source_bytes)
    if target.read_bytes() != source_bytes:
        raise CapabilityInputError("INSO resolver alias is not byte-identical to official source")
    source_sha = hashlib.sha256(source_bytes).hexdigest()
    alias_sha = sha256_file(target)
    if alias_sha != source_sha or target.stat().st_size != source.stat().st_size:
        raise CapabilityInputError("INSO resolver alias digest/size drift")
    return {
        "schemaVersion": 1,
        "status": "BYTE_IDENTICAL_OPAC_SPECIES_RESOLVER_ALIAS_CREATED",
...
def render_disort_input(data_dir: Path, repository_root: Path, profile_path: Path) -> str:
    lines = [*_common_lines(data_dir, repository_root, DISORT_SZA_DEG, profile_path), "rte_solver disort", "number_of_streams 16", "output_user lambda uu", "quiet"]
    text = "\n".join(lines) + "\n"
...
def render_mystic_input(data_dir: Path, repository_root: Path, profile_path: Path, run_dir: Path) -> str:
    lines = [*_common_lines(data_dir, repository_root, MYSTIC_SZA_DEG, profile_path), "rte_solver mystic", "mc_spherical 1D", f"mc_photons {MYSTIC_PHOTONS}", "mc_vroom on", "mc_std", f"mc_randomseed {MYSTIC_SEED}", f"mc_basename {(run_dir / 'mc').resolve()}", "quiet"]
    text = "\n".join(lines) + "\n"
...
def write_bundle(atmosphere_path: Path, data_dir: Path, repository_root: Path, output_root: Path) -> dict:
    ...
    meta = {
        "schemaVersion": 1,
        "stageId": STAGE_ID,
        "status": "FROZEN_SYNTHETIC_CAPABILITY_INPUTS_WITH_BYTE_IDENTICAL_OPAC_ALIAS",
        "species": SPECIES,
        "speciesReason": "single insoluble OPAC species; synthetic capability only",
        "resolverAlias": alias,
        "aod550": AOD550,
