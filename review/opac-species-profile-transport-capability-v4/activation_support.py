...
def validate_runtime_reports(base: dict[str, Any], pre: dict[str, Any], post: dict[str, Any], alias: dict[str, Any]) -> dict[str, Any]:
    ...
    if not alias.get("byteIdentical") or alias.get("sourceSha256") != alias.get("aliasSha256") or int(alias.get("byteCount") or 0) <= 0:
        raise RuntimeError("resolver alias provenance drift")
    if alias.get("aliasRelativePath") != "aerosol/OPAC/INSO.nc":
        raise RuntimeError("resolver alias is not at OPAC library root")
    ...
    return {"schemaVersion": 1, "status": "POST_ALIAS_RUNTIME_PROVENANCE_VALIDATED", "preAliasDataTreeSha256": pre["libRadtranDataTreeSha256"], "postAliasDataTreeSha256": post["libRadtranDataTreeSha256"], "resolverAlias": alias}
...
def freeze_capability_report(evidence: Path, run_id: int) -> dict[str, Any]:
    ...
    out = {
        "schemaVersion": 1,
        "stageId": "opac-species-profile-transport-capability-v4",
        "status": "PASS_LIBRARY_ROOT_ALIAS_REACHES_DISORT_AND_MYSTIC" if disort_diff and mystic_diff else "FAILED_BEFORE_REQUIRED_COMPARISONS",
