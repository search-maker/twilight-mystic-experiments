...
STAGE_ID = "opac-species-profile-transport-capability-v4"
SPECIES = "INSO"
AOD550 = 0.10
DISORT_SZA_DEG = 80.0
MYSTIC_SZA_DEG = 96.0
...
ALIAS_TARGET_REL = Path("aerosol/OPAC/INSO.nc")
...
def prepare_inso_alias(data_dir: Path) -> dict:
    ...
    if target.exists():
        raise CapabilityInputError(f"INSO library-root alias unexpectedly preexists: {target}")
    target.parent.mkdir(parents=True, exist_ok=True)
    source_bytes = source.read_bytes()
    target.write_bytes(source_bytes)
    if target.read_bytes() != source_bytes:
        raise CapabilityInputError("INSO library-root alias is not byte-identical to official source")
    alias_sha = sha256_file(target)
    if alias_sha != EXPECTED_SOURCE_SHA256 or target.stat().st_size != source.stat().st_size:
        raise CapabilityInputError("INSO library-root alias digest/size drift")
    return {
        "schemaVersion": 1,
        "status": "BYTE_IDENTICAL_OPAC_LIBRARY_ROOT_ALIAS_CREATED",
...
def parse_afgl_heights_km(path: Path) -> tuple[float, ...]:
    ...
    if len(rows) < 5 or not all(rows[i] > rows[i + 1] for i in range(len(rows) - 1)) or rows[-1] > 1e-9:
        raise CapabilityInputError("unexpected AFGL altitude grid")
    return tuple(rows)
...
def write_bundle(atmosphere_path: Path, data_dir: Path, repository_root: Path, output_root: Path) -> dict:
    ...
    meta = {
        "schemaVersion": 1,
        "stageId": STAGE_ID,
        "status": "FROZEN_SYNTHETIC_CAPABILITY_INPUTS_WITH_LIBRARY_ROOT_ALIAS",
        "species": SPECIES,
        "resolverAlias": alias,
        "aod550": AOD550,
