...
EXPECTED_ALIAS_REL = "aerosol/OPAC/optprop/INSO"
TRACE_RUN_ID = 33185460954
TRACE_ARTIFACT_ID = 9691518729
TRACE_ARTIFACT_DIGEST = "sha256:07fb60de7bef96253eaf29cb9303a83bab7f3f1952431c73a26499357b4d572a"
...
def validate_runtime_reports(base: dict[str, Any], pre: dict[str, Any], post: dict[str, Any], alias: dict[str, Any]) -> dict[str, Any]:
    ...
    if alias.get("sourceSha256") != EXPECTED_SOURCE_SHA256 or alias.get("aliasSha256") != EXPECTED_SOURCE_SHA256:
        raise RuntimeError("resolver alias SHA provenance drift")
    if alias.get("aliasRelativePath") != EXPECTED_ALIAS_REL:
        raise RuntimeError("resolver alias does not match trace-observed no-extension path")
    if alias.get("traceEvidenceRunId") != TRACE_RUN_ID or alias.get("traceEvidenceArtifactId") != TRACE_ARTIFACT_ID:
        raise RuntimeError("trace evidence identity drift")
    if alias.get("traceEvidenceArtifactDigest") != TRACE_ARTIFACT_DIGEST:
        raise RuntimeError("trace evidence digest drift")
    if int(alias.get("byteCount") or 0) <= 0:
        raise RuntimeError("resolver alias byte count invalid")
    ...
    return {
        "schemaVersion": 1,
        "status": "POST_TRACE_OBSERVED_ALIAS_RUNTIME_PROVENANCE_VALIDATED",
...
def validate_output_pair(low: Path, high: Path, label: str) -> dict[str, Any]:
    ...
    if low_sha == high_sha:
        raise RuntimeError(f"{label} LOW/HIGH outputs are byte-identical")
    if low_rows == high_rows:
        raise RuntimeError(f"{label} LOW/HIGH numeric rows are identical")
    return {
        "label": label,
        "rowCount": len(low_rows),
        "grid": list(low_grid),
        "lowSha256": low_sha,
        "highSha256": high_sha,
        "outputsDiffer": True,
...
def freeze_capability_report(evidence: Path, run_id: int) -> dict[str, Any]:
    ...
    out = {
        "schemaVersion": 1,
        "stageId": "opac-species-profile-transport-capability-v5",
        "status": "PASS_TRACE_OBSERVED_ALIAS_REACHES_DISORT_AND_MYSTIC" if disort_diff and mystic_diff else "FAILED_BEFORE_REQUIRED_COMPARISONS",
