...
def validate_data(d:dict[str,Any])->None:
    ...
    req(s.get('precisionExhaustedTreatmentSha256')==EXHAUSTED and s.get('train0037PreregistrationSha256')==TRAIN0037,'legacy treatment binding drift')
    ...
    req(sl.get('globalCollisionAuditStillRequiredBeforeAuthorization') is True and sl.get('collisionOutcome')=='VERSION_NEW_SEED_LEDGER_AND_CAMPAIGN_CONTRACT_BEFORE_ANY_RESULT_OPENING','seed collision guard drift')
