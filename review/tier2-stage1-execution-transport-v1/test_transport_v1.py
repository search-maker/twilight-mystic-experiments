...
def test_adapter_and_executor():
    with tempfile.TemporaryDirectory() as td:
        ...
        old={k:os.environ.get(k) for k in ['GITHUB_ACTIONS','GITHUB_EVENT_NAME','GITHUB_RUN_ATTEMPT','GITHUB_REF_NAME']}; os.environ.update({'GITHUB_ACTIONS':'true','GITHUB_EVENT_NAME':'push','GITHUB_RUN_ATTEMPT':'1','GITHUB_REF_NAME':'dispatch/tier2-stage1-ordinal19-v3'})
        try:r=X.execute_case(E/'stage1-execution-manifest-v1.json',rp,E/'adapter_v1.py',case['caseId'],data,repo,u,out2,900,True,'dispatch/tier2-stage1-ordinal19-v3',runner)
        finally:
...
def test_context_refusal():
    try:X.validate_context(True,'dispatch/tier2-stage1-ordinal19-v3')
