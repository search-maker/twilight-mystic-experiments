...
assert "json.dumps({'include':rows},separators=(',',':'))" in exe
assert "json.dumps(rows,separators=(',',':'))" not in exe
assert 'matrix: ${{ fromjson(needs.preflight.outputs.matrix) }}' in exe
assert 'assert len(rows)==76' in exe
