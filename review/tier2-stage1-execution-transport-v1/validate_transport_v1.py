...
def validate(d:dict[str,Any],repo_root:Path|None=None)->None:
    ...
    s=d.get('sourceBindings') or {}; req(s.get('liveMainAtFreeze')==MAIN and s.get('latestConsumedScientificOrdinal')==18,'main/ordinal drift'); req(s.get('campaignContractSha256')==CAM and s.get('stage1ImplementationSha256')==IMP and s.get('artifactReplayResultSha256')==REP,'source selfhash binding drift')
