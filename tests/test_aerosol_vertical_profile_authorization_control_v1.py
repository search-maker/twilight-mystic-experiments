...
class AerosolVerticalProfileAuthorizationControlV1Tests(unittest.TestCase):
    def test_seeded_design_is_exact_nonrenderable_360_case_universe(self) -> None:
        design_mod = load("avps_design_test", DESIGN_PATH)
        main = "a" * 40
        design = design_mod.build_review_execution_design(seed_proof(main), main)
        self.assertEqual((design["caseCount"], design["groupCount"], design["statesPerGroup"]), (360, 72, 5))
    ...
    def test_builder_and_guard_freeze_pending_dispatch_document(self) -> None:
        builder = load("avps_builder_test", BUILDER_PATH)
        guard = load("avps_guard_test", GUARD_PATH)
        main = "a" * 40; report = preauth_report(main, 40); proof = seed_proof(main)
    ...
    def test_review_requires_exact_one_file_draft_direct_child_attempt1(self) -> None:
        ...
        main = "a" * 40; head = "b" * 40; report = preauth_report(main, 40); proof = seed_proof(main)
        kwargs = dict(preauthorization_artifact_id=999, preauthorization_artifact_digest="sha256:" + "d" * 64)
        auth = builder.build(ROOT, main, 40, report, proof, **kwargs)
        ctx = {
    ...
    def test_workflow_zero_runtime_and_refresh_anchor_is_watched(self) -> None:
        text = WORKFLOW.read_text()
        for required in ("types: [opened]", "test \"$GITHUB_RUN_ATTEMPT\" = 1", "vertical-profile-v1-preauthorization-proof", "authorization-review-evidence", "candidateSeedAuthorizationRecheckPassed"):
        ...
        self.assertIn("Authorization-control generation v1", SURFACE_PATH.read_text())
