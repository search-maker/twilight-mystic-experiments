...
def write_fixture(root: Path) -> None:
    for _role, (rel, tokens) in A.SURFACES.items():
        path = root / rel
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text("\n".join(tokens) + "\n", encoding="utf-8")
    runner = root / A.RUNNER_PATH
    runner.parent.mkdir(parents=True, exist_ok=True)
    runner.write_text("\n".join(A.RUNNER_REQUIRED_TOKENS) + "\n", encoding="utf-8")


class SuccessorBindingAuditTests(unittest.TestCase):
    def test_actual_repository_tree_freezes_complete_rebind_surface(self):
        out = A.audit_source_tree(ROOT)
        self.assertEqual(out["status"], A.STATUS)
        self.assertEqual(out["legacy_case_id"], "2017-06-16_dusk")
        self.assertEqual(set(out["legacy_rebind_surfaces"]), set(A.SURFACES))
        self.assertTrue(out["lower_frozen_runner"]["selected_case_passthrough_capable"])
        self.assertFalse(out["lower_frozen_runner"]["legacy_case_hardpin_present"])
    ...
    def test_refuses_missing_legacy_binding_token(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            write_fixture(root)
            rel, tokens = A.SURFACES["portable_wrapper"]
            path = root / rel
            path.write_text(tokens[0] + "\n", encoding="utf-8")
            with self.assertRaises(A.AuditRefusal):
                A.audit_source_tree(root)

    def test_refuses_lower_runner_that_acquires_legacy_case_hardpin(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            write_fixture(root)
            runner = root / A.RUNNER_PATH
            runner.write_text(
                "\n".join(A.RUNNER_REQUIRED_TOKENS + A.RUNNER_FORBIDDEN_TOKENS) + "\n",
                encoding="utf-8",
            )
            with self.assertRaises(A.AuditRefusal):
                A.audit_source_tree(root)

    def test_refuses_missing_required_surface(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            write_fixture(root)
            rel, _tokens = A.SURFACES["authorized_run_envelope"]
            (root / rel).unlink()
            with self.assertRaises(A.AuditRefusal):
                A.audit_source_tree(root)
    ...
    def test_report_preregisters_no_science_authority(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            write_fixture(root)
            out = A.audit_source_tree(root)
            self.assertGreaterEqual(len(out["successor_requirements"]), 6)
            for key in (
