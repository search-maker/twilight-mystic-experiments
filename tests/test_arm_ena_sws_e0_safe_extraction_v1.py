...
def make_zip_with_mode(path: Path, mode: int) -> tuple[int, str]:
    with zipfile.ZipFile(path, "w", compression=zipfile.ZIP_DEFLATED) as archive:
...
class SafeExtractionTests(unittest.TestCase):
    ...
    def test_permission_only_unix_mode_is_accepted(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            archive_path = root / "permission-only.zip"
            size, digest = make_zip_with_mode(archive_path, 0o600)
            result = V.extract_safely(digest_receipt(size, digest), archive_path, root / "out")
            self.assertEqual(result["status"], V.STATUS)

    def test_explicit_regular_unix_mode_is_accepted(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            archive_path = root / "regular.zip"
            size, digest = make_zip_with_mode(archive_path, stat.S_IFREG | 0o600)
            result = V.extract_safely(digest_receipt(size, digest), archive_path, root / "out")
            self.assertEqual(result["status"], V.STATUS)
    ...
    def test_symlink_member_is_refused(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            size, digest = make_zip_with_mode(archive_path, stat.S_IFLNK | 0o777)
            with self.assertRaisesRegex(V.SafeExtractionError, "non-regular"):
                V.extract_safely(digest_receipt(size, digest), archive_path, root / "out")

    def test_other_explicit_nonregular_unix_mode_is_refused(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            archive_path = root / "fifo.zip"
            size, digest = make_zip_with_mode(archive_path, stat.S_IFIFO | 0o600)
            with self.assertRaisesRegex(V.SafeExtractionError, "non-regular"):
                V.extract_safely(digest_receipt(size, digest), archive_path, root / "out")
