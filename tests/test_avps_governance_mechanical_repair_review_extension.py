...
class AvpsGovernanceMechanicalReviewerExtensionContract(unittest.TestCase):
    ...
    def test_begin_anchor_matcher_is_indentation_agnostic_exact_statement_only(self):
        required = (
            'old_begin_line = re.compile(',
            'new_begin_line = re.compile(',
            r"(?m)^(?P<indent>[ \t]*)elif first\.startswith\('WRITE_QUIET_BEGIN'\):$",
            r'(?m)^(?P<indent>[ \t]*)elif is_write_quiet_begin\(body\):$',
            'def begin_counts(text):',
            'def canonicalize_begin(text):',
            "match.group('indent')",
            'mixed-indentation old BEGIN fixture failed',
            'mixed-indentation owner-aware BEGIN fixture failed',
            'mixed-indentation BEGIN canonicalization fixture failed',
            'non-exact BEGIN lookalike unexpectedly counted',
            'non-exact BEGIN lookalike unexpectedly normalized',
        )
        for token in required:
            self.assertIn(token, self.reviewer)
        self.assertNotIn("old_count = text.count(old_begin)", self.reviewer)
        self.assertNotIn("new_count = text.count(new_begin)", self.reviewer)
        self.assertNotIn('text = text.replace(old_begin, canonical_begin)', self.reviewer)
        self.assertNotIn('text = text.replace(new_begin, canonical_begin)', self.reviewer)
        self.assertIn("                  elif first.startswith('WRITE_QUIET_BEGIN'):\\n", self.reviewer)
        self.assertIn('if begin_counts(frozen_layout_old) != (3, 0):', self.reviewer)
        self.assertIn("                  elif is_write_quiet_begin(body):\\n", self.reviewer)
        self.assertIn('if begin_counts(frozen_layout_new) != (0, 3):', self.reviewer)
        self.assertIn("elif first.startswith('WRITE_QUIET_BEGIN'): # lookalike\\n", self.reviewer)
