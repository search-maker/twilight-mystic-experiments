...
class WriteQuietParserRepairContract(unittest.TestCase):
    def test_canonical_parser_exports_historical_surface(self):
        self.assertTrue(callable(write_quiet_end_binding))
        self.assertTrue(callable(record_write_quiet_end))
        self.assertTrue(callable(is_write_quiet_begin))
    ...
    def test_repeated_identical_authoritative_bindings_canonicalize(self):
        good=(
            ...
            'TOTAL_SKY_OWNER::WRITE_QUIET_END | stage=x | beginComment=5573615763\nbeginComment=5573615763',
        )
        for body in good:
            with self.subTest(body=body):
                expected=5573615763 if '5573615763' in body else 123
                self.assertEqual(write_quiet_end_binding(body),expected)

    def test_historical_multiline_and_legacy_space_delimited_end(self):
        ...
        self.assertEqual(write_quiet_end_binding('TOTAL_SKY_OWNER::WRITE_QUIET_END | stage=x | beginComment=123'),123)
    ...
    def test_end_marker_grammar_is_first_line_exact(self):
        for body in (
            ...
            'prose WRITE_QUIET_END | beginComment=123',
            'header\n> WRITE_QUIET_END | beginComment=123',
            'header\n`WRITE_QUIET_END | beginComment=123`',
        ):
            with self.subTest(body=body):
    ...
    def test_begin_grammar_is_first_line_exact_and_owner_aware(self):
        ...
        for body in ('prose WRITE_QUIET_BEGIN | stage=x','Total_Sky_OWNER::WRITE_QUIET_BEGIN | stage=x','TOTAL-SKY-OWNER::WRITE_QUIET_BEGIN | stage=x','header\nTOTAL_SKY_OWNER::WRITE_QUIET_BEGIN | stage=x'):
            with self.subTest(body=body):
    ...
    def test_mismatched_and_true_duplicate_bindings_fail_closed(self):
        closed=set()
        self.assertTrue(record_write_quiet_end('WRITE_QUIET_END | stage=x\nbeginComment=123',200,{123},closed))
    ...
    def test_exact_superseded_predecessor_retains_provenance_and_canonicalizes(self):
    ...
    def test_publisher_is_migrated_to_canonical_module(self):
        text=_workflow_text()
