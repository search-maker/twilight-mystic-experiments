...
class LowAltState0002Exec001Tests(unittest.TestCase):
    ...
    def test_controller_binding_and_fresh_universe(self) -> None:
        ...
        self.assertEqual(m["directTransportContract"]["molecularAbsorption"], "crs")
        self.assertEqual(mod.MOL_ABS_PARAM, "crs")
        self.assertEqual(m["inheritedSeam"]["geometricAltitudeDeg"], "5.0")
        self.assertFalse(m["inheritedSeam"]["freshSuccessorEvidence"])
        self.assertTrue(m["inheritedSeam"]["mustRemainV32Authoritative"])
    ...
    def test_renderer_is_geometric_sdisort_and_site_truncated(self) -> None:
        ...
        self.assertEqual(text.count("mol_abs_param crs"), 1)
        self.assertNotIn("mol_abs_param reptran", text.lower())
