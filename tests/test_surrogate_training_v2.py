...
class SurrogateTrainingV2Tests(unittest.TestCase):
    ...
    def test_adapter_accepts_complete_partition_and_refuses_hash_tamper(self):
        ...
        partitioned = adapter.read_tier1_dataset(self.paths["dataset"], self.paths["envelope"], self.paths["design"], expected_main_sha="e7c7b0e1bef4f8b3e3989e7ed445a008846ac914")
        ...
        with self.assertRaises(adapter.DatasetRefusal):
            adapter.read_tier1_dataset(self.paths["dataset"], self.paths["envelope"], self.paths["design"], expected_main_sha="e7c7b0e1bef4f8b3e3989e7ed445a008846ac914")

    def test_adapter_refuses_overlap_role_change_and_adaptive_geometry(self):
        self.make_adapter_eligible()
    ...
    def test_candidate_protocol_is_frozen_and_small(self):
        ...
        self.assertEqual([item["candidateId"] for item in self.protocol["candidates"]], ["transparent-log-mean-baseline", "fixed-basis-log-ridge", "local-log-idw"])
    ...
    def test_model_freezes_before_holdout_and_reports_anchors_separately(self):
        self.make_adapter_eligible()
        partitioned = adapter.read_tier1_dataset(self.paths["dataset"], self.paths["envelope"], self.paths["design"], expected_main_sha="e7c7b0e1bef4f8b3e3989e7ed445a008846ac914")
        model, artifact = training.freeze_artifact(self.protocol, partitioned, {"adapter.py": "a" * 64, "training.py": "b" * 64, "candidate-protocol.json": "c" * 64})
        ...
        external = training.evaluate_external(model, artifact, list(partitioned.hard_anchors), list(partitioned.soft_diagnostics))
