...
The same exact title must also be accepted as `allowed` by the repository's current `tools/arm_ena_sws_query_only_availability_v1/stress.py` direct-ARM classifier. This preregisters against another pre-query self-poison: a semantically positive title that the executable stress gate would classify as ambiguous cannot be used by the guarded successor plan. A future exact governance request should therefore use an explicit stress-compatible marker such as `SUCCESSOR_AUTHORIZED` and still be checked against the then-current exact classifier before dispatch.

This prevents an owner request, stale allowed control, unrelated Coordinator line, or stress-incompatible positive-looking title from being relabeled after the fact as query authority. Ledger bodies are used only for exact provenance verification and are never copied into the emitted rebind plan. The guarded plan remains result-blind and non-authorizing: it performs no ARM query/download, reads no credential values, opens no protected SWS/SASZE values, applies no source rebind, and grants no E0 execution, held-out opening, Stage B, MYSTIC/science, or production authority.

## Live pre-dispatch control-plane binding v2

`preflight_query_authorization_v1.py` validates a supplied complete Issue #60 snapshot, fresh issue-tail identity, fresh current-main SHA, exact Coordinator title/classifier disposition, and dedicated workflow/event/main/one-shot/attempt-1 binding lines before any future authenticated query-only dispatch. Its receipt is control-plane-only and cannot authorize E0 or science.

`preflight_query_authorization_live_v2.py` removes the remaining caller-supplied freshness ambiguity. It refuses to run if either ARM credential is present, reads only GitHub control-plane state with `GITHUB_TOKEN`, and obtains the current repository/default-branch/main and Issue #60 ledger itself. It double-reads the default-branch main SHA, double-reads Issue #60 metadata, and independently fetches the entire ordered Issue #60 comment ledger twice. Both complete `{id,body}` projections must be byte-canonical-equivalent and hash-identical, while the issue count/`updated_at` and main SHA must also remain stable. The stable live snapshot is then passed into the v1 binding gate with the live current main; there is no caller-provided main or comments file to launder a stale authorization.

This double-ledger rule is intentionally stronger than comment-count/latest-ID checks alone: an edit to an existing control comment, a same-count identity/body drift, an add/delete, or a concurrent main movement fails closed even if coarse issue metadata would otherwise appear unchanged. Unit refusals cover main movement, issue metadata drift, body or identity changes between complete snapshots, count mismatch, default-branch drift, stale authorization-body main binding, ARM-credential presence, and missing GitHub control-plane credentials.

A `QUERY_ONLY_SUCCESSOR_LIVE_PREDISPATCH_GOVERNANCE_PASS` receipt proves only that a stable, current GitHub control-plane view satisfied the preregistered governance checks at that instant. It does not dispatch, query or download ARM, does not read ARM credential values, does not open native/protected data, and does not grant E0, held-out, Stage B, MYSTIC/science, or production authority.
