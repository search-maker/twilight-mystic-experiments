...
POSITIVE_TOKEN = 'PHYSICS_INDEPENDENT::EXEC005_V5R8_EXACT_BYTES_ACCEPTED_FOR_SINGLE_NON_SCIENCE_LIVE_CONTROL'
PREFIX = 'PHYSICS_INDEPENDENT::EXEC005_V5R8'
...
def mutable_inventory(document: Any, current_run_id: int) -> dict[str, Any]:
    ...
    return {
        'schema': 'LUNAR_EXEC005_V5R8_COMPLETE_UNFILTERED_ACTIONS_INVENTORY_V1',
...
def inventories_stable(left: dict[str, Any], right: dict[str, Any]) -> bool:
    return left.get('canonicalMutableSha256') == right.get('canonicalMutableSha256') and left.get('canonicalMutableRows') == right.get('canonicalMutableRows')
...
def classify_physics(comments: list[dict[str, Any]], *, cutoff: int, head: str, tree: str, branch: str, artifact_id: int) -> dict[str, Any]:
    ...
    authorized = len(positive_ids) == 1 and not rejection_ids and not malformed_ids and not ambiguous_ids
    return {
        'schema': 'LUNAR_EXEC005_V5R8_EXACT_PHYSICS_BINDING_V1',
...
def _run(run_id: int, status: str, *, path: str = 'fixture', conclusion: Any = None) -> dict[str, Any]:
    return {'id': run_id, 'status': status, 'conclusion': conclusion, 'run_attempt': 1, 'head_sha': 'a' * 40, 'head_branch': 'fixture', 'path': path, 'event': 'push'}
...
def self_test() -> dict[str, Any]:
    ...
    same_comment_reject = classify_physics([{'id': 5001, 'body': good_body + '\nREJECTED because fixture'}], cutoff=5000, head=head, tree=tree, branch=branch, artifact_id=artifact)
    duplicate_positive = classify_physics([{'id': 5001, 'body': good_body}, {'id': 5002, 'body': good_body}], cutoff=5000, head=head, tree=tree, branch=branch, artifact_id=artifact)
    duplicate_binding = classify_physics([{'id': 5003, 'body': f'{POSITIVE_TOKEN}\n{identity}\nhead={head}'}], cutoff=5000, head=head, tree=tree, branch=branch, artifact_id=artifact)
    missing_binding = classify_physics([{'id': 5004, 'body': f'{POSITIVE_TOKEN}\nbranch={branch}\nhead={head}\ntree={tree}'}], cutoff=5000, head=head, tree=tree, branch=branch, artifact_id=artifact)
    wrong_binding = classify_physics([{'id': 5005, 'body': f'{POSITIVE_TOKEN}\nbranch={branch}\nhead={"3"*40}\ntree={tree}\nartifact={artifact}'}], cutoff=5000, head=head, tree=tree, branch=branch, artifact_id=artifact)
    quoted_context = classify_physics([{'id': 5006, 'body': f'{POSITIVE_TOKEN}\n> branch={branch}\n> head={head}\n> tree={tree}\n> artifact={artifact}'}], cutoff=5000, head=head, tree=tree, branch=branch, artifact_id=artifact)
    legacy_alias = classify_physics([{'id': 5007, 'body': f'{PREFIX}_EXACT_BYTE_ACCEPTED\n{identity}'}], cutoff=5000, head=head, tree=tree, branch=branch, artifact_id=artifact)
    return {
        'schema': 'LUNAR_EXEC005_V5R8_CONTROL_GUARD_SELFTEST_V1',
